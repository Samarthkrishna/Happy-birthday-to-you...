// ===== DOM ELEMENTS =====
let nameInput, activateBtn, dynamicMessage, mainTitle, confettiBtn, laserBtn;
let playBtn, pauseBtn, nextBtn, volumeSlider, loveBtn, loveMeter, loveCount;
let surpriseBtn, surpriseOutput, shareBtn, resetBtn, viewCount;
let daysElement, hoursElement, minutesElement, secondsElement;

// Audio elements
let bgMusic, laserSound, confettiSound, clickSound;

// Containers
let particleBg, starsContainer, confettiContainer, laserContainer;
...
function setup() {
    ...
    dynamicMessage = document.getElementById('dynamicMessage');
    ...
    surpriseOutput = document.getElementById('surpriseOutput');
    ...
    laserContainer = document.getElementById('laserContainer');

    // Check if elements exist
    if (!nameInput || !activateBtn) {
    ...
    if (viewCount) viewCount.textContent = viewCountNumber;
...
function activateCelebration() {
    if (!nameInput || !dynamicMessage || !mainTitle) return;
    ...
    if (userName) {
        ...
        const titleText = mainTitle.querySelector('.title-text');
        if (titleText) {
            titleText.textContent = `HAPPY BIRTHDAY ${userName.toUpperCase()}`;
        }
...
function showWish(index) {
    if (!dynamicMessage || index < 0 || index >= wishes.length) return;
...
function sendLove() {
    if (!loveCount || !loveMeter || !dynamicMessage) return;
    ...
    const meterFill = loveMeter.querySelector('.meter-fill');
    if (meterFill) {
...
function resetSimulation() {
    ...
    if (loveCount) loveCount.textContent = '0';
    if (loveMeter) {
        const meterFill = loveMeter.querySelector('.meter-fill');
        if (meterFill) meterFill.style.width = '0%';
    }
    if (nameInput) nameInput.value = '';
    if (dynamicMessage) dynamicMessage.textContent = "System reset. Ready for new celestial celebration.";
    
    const mainTitle = document.getElementById('mainTitle');
    if (mainTitle) {
        const titleText = mainTitle.querySelector('.title-text');
        if (titleText) titleText.textContent = "HAPPY BIRTHDAY";
    }
    
    if (surpriseOutput) surpriseOutput.innerHTML = '';
